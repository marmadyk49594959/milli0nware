...
public:
    ...
    //-----------------------------------------------------------------------------
    void init(const vector_t &x_axis, const vector_t &y_axis, const vector_t &z_axis, const vector_t &vec_origin) {
    ...
    // the Y axis = left, and the Z axis = up
    //-----------------------------------------------------------------------------
    matrix3x4_t(const vector_t &x_axis, const vector_t &y_axis, const vector_t &z_axis, const vector_t &vec_origin) {
        init(x_axis, y_axis, z_axis, vec_origin);
    }

    void set_origin(const vector_t &p) {
        m_mat_val[0][3] = p.x;
        m_mat_val[1][3] = p.y;
        m_mat_val[2][3] = p.z;
    }
